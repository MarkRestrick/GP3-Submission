...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		mouseXI = GET_X_LPARAM(lParam);
		mouseYI = GET_Y_LPARAM(lParam);



		mouseX = (float)(mouseXI - oldMouseXI);
		mouseY = (float)(mouseYI - oldMouseYI);
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_LEFT || wParam == 'a' || wParam == 'A') //If the Left Arrow key was pressed 
		{
			//rotationAngle = rotationAngle + 5; //Increase rotation Angle 
			//glRotatef(rotationAngle, 0.0f, 1.0f, 0.0f);
			camLeft = true;
		}
		if (wParam == VK_RIGHT || wParam == 'd' || wParam == 'D') //If the Left Arrow key was pressed 
		{
			//rotationAngle = rotationAngle - 5; //Decrease rotation Angle 
			//glRotatef(-rotationAngle, 0.0f, 1.0f, 0.0f);
			camRight = true;
		}
		if (wParam == VK_DOWN || wParam == 's' || wParam == 'S') //If the Left Arrow key was pressed 
		{
			camBack = true;
		}
...
void cWNDManager::SetCamForward()
{
	camFor = true;
}
